...
import {
  ...
  Megaphone, Lightbulb, ArrowRight, BarChart3, UserCheck,
} from 'lucide-react';
...
const shortcuts = [
  ...
  },
  {
    icon: Users,
    title: 'Pacientes',
    description: 'Gerencie seus pacientes e fichas clínicas',
    path: '/app/pacientes',
  },
  {
    icon: Clock,
    title: 'Histórico',
    description: 'Veja todas as evoluções clínicas geradas',
    path: '/app/historico',
  },
  {
    icon: Megaphone,
    title: 'IA de Marketing',
    description: 'Crie conteúdo para redes sociais e divulgação',
    path: '/marketing',
  },
];
...
const HomePage = () => {
  ...
  if (patientsCount === 0) {
    tips.push({ text: 'Cadastre seu primeiro paciente para organizar suas evoluções', path: '/app/pacientes' });
  }
  if (evolutionsCount === 0) {
    tips.push({ text: 'Experimente gerar sua primeira evolução clínica', path: '/app/evolucao' });
  }
  if (patientsCount > 0 && evolutionsCount > 0) {
    tips.push({ text: 'Use o Chat Clínico para discutir casos complexos com a IA', path: '/chat' });
  }
  ...
  const getHeroContent = () => {
    if (patientsCount === 0 && !loadingStats) {
      return {
        title: 'Comece cadastrando seus pacientes',
        description: 'Organize suas fichas clínicas para gerar evoluções personalizadas com IA',
        ctaLabel: 'Cadastrar paciente',
        ctaPath: '/app/pacientes',
        secondaryCta: null,
      };
    }
    if (evolutionsCount > 0 && !loadingStats) {
      return {
        title: 'Gerar Evolução Clínica',
        description: `Você gerou ${evolutionsCount} ${evolutionsCount === 1 ? 'evolução' : 'evoluções'} este mês. Continue documentando suas sessões!`,
        ctaLabel: 'Começar agora',
        ctaPath: '/app/evolucao',
        secondaryCta: { label: 'Ver histórico', path: '/app/historico' },
      };
    }
    return {
      title: 'Gerar Evolução Clínica',
      description: 'Transforme suas anotações de sessão em documentação clínica completa com IA',
      ctaLabel: 'Começar agora',
      ctaPath: '/app/evolucao',
      secondaryCta: null,
    };
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
          ...
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              {loadingStats ? (
                ...
                <p className="text-2xl font-bold text-foreground">{evolutionsCount}</p>
              )}
          ...
          <CardContent className="p-5 space-y-3">
            ...
            <ul className="space-y-2">
              {tips.slice(0, 2).map((tip, i) => (
                <li
                  key={i}
                  className="flex items-center gap-3 text-sm text-primary cursor-pointer hover:underline"
      ...
      {/* Referral — compact at the bottom */}
      <ReferralCard />
    </div>
  );
};
